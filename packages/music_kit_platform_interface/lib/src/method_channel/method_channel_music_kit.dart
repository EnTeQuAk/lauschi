...
class MethodChannelMusicKit extends MusicKitPlatform {
  ...
  @visibleForTesting
  final playerStateEventChannel = const EventChannel(
    'plugins.misi.app/music_kit/player_state',
  );

  @visibleForTesting
  final playerQueueEventChannel = const EventChannel(
    'plugins.misi.app/music_kit/player_queue',
  );

  @visibleForTesting
  final musicSubcriptionEventChannel = const EventChannel(
  ...
  Future<String> requestDeveloperToken() async {
    final resp = await methodChannel.invokeMethod<String>(
  ...
  Stream<MusicSubscription> get onSubscriptionUpdated {
    _onSubscriptionUpdated ??= musicSubcriptionEventChannel
  ...
  Stream<MusicPlayerState> get onMusicPlayerStateChanged {
    _onMusicPlayerStateChanged ??= playerStateEventChannel
        .receiveBroadcastStream()
        .map((event) {
          final json = event.cast<String, dynamic>();
  ...
  Future<void> setQueue(String type, {required ResourceObject item}) {
    return methodChannel.invokeMethod('setQueue', {'type': type, 'item': item});
  }
  ...
  Stream<MusicPlayerQueue> get onPlayerQueueChanged {
    _onPlayerQueueChanged ??= playerQueueEventChannel
        .receiveBroadcastStream()
        .map((event) {
...
MusicAuthorizationStatus _authorizationStatusfromRawValue(
  int rawValue, {
