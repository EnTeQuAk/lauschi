...
public class MusicKitPlugin: NSObject, FlutterPlugin {
  ...
  public static func register(with registrar: FlutterPluginRegistrar) {
    ...
    // lauschi: unified playback state stream (position, duration, track changes).
    // Uses the same channel name as Android's DRM state for Dart compatibility.
    let lauschiPlaybackChannel = FlutterEventChannel(
      name: "plugins.misi.app/music_kit/drm_player_state",
      binaryMessenger: messenger
    )
    lauschiPlaybackChannel.setStreamHandler(LauschiPlaybackStreamHandler(musicPlayer: instance.musicPlayer))
  }
  ...
  public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
    ...
    case .setQueueWithStoreIds:
      let arguments = call.arguments as! JSONObject
      setQueueWithStoreIds(
        ids: arguments["ids"] as! [String],
        startingAt: arguments["startingAt"] as? Int,
        result: result
      )
    }
