...
class TestFactsRoundTrip:
    ...
    def test_re_verify_changes_mind(self):
        """A fact that was previously agreed can be disagreed on re-verify.

        The stale confirmed_by/confirmed_at must be cleared so
        _filter_confirmed_facts doesn't ship it to yaml.
        """
        import json
        from datetime import UTC, datetime
        from pathlib import Path

        from lauschi_catalog.commands.verify import apply_verification, FactVerdict, VerifyResult

        tmp_dir = Path("/tmp/test_verify_mind_change")
        tmp_dir.mkdir(exist_ok=True)
        curation_path = tmp_dir / "mind_change.json"
        curation = {
            "id": "mind_change",
            "title": "Mind Change",
            "albums": [],
        ...
        try:
            result = VerifyResult(
                series_id="mind_change",
                approve=True,
                fact_verdicts=[
                    FactVerdict(fact_type="era_boundary", identifier="shaky", agree=False, reason="dates don't match"),
                ],
            )
            action = apply_verification("mind_change", result, dry_run=False)
            assert action == "approved"

            updated = json.loads(curation_path.read_text())
            era = updated["series_facts"]["era_boundaries"][0]
            assert era["confirmed_by"] is None
            assert era["confirmed_at"] is None
            assert era["verify_status"] == "disagreed"
            assert era["verify_reasoning"] == "dates don't match"

            # yaml filter also rejects it
            from lauschi_catalog.commands.apply import _filter_confirmed_facts
            assert _filter_confirmed_facts(updated["series_facts"]) is None
        finally:
            verify_mod.CURATION_DIR = orig_dir

    def test_string_identifier_coerced_for_known_gap(self):
        """LLM may emit identifier:"156" (string) when number is int 156."""
        import json
        from pathlib import Path

        from lauschi_catalog.commands.verify import apply_verification, FactVerdict, VerifyResult

        tmp_dir = Path("/tmp/test_string_id")
        tmp_dir.mkdir(exist_ok=True)
        curation_path = tmp_dir / "string_id.json"
        curation = {
            "id": "string_id",
            "title": "String ID",
            "albums": [],
        ...
        try:
            result = VerifyResult(
                ...
                approve=True,
                fact_verdicts=[
                    FactVerdict(fact_type="known_gap", identifier="156", agree=True),
                ],
            )
            action = apply_verification("string_id", result, dry_run=False)
            updated = json.loads(curation_path.read_text())
            gap = updated["series_facts"]["known_gaps"][0]
            assert gap["confirmed_by"] == "verify"
            assert gap["verify_status"] == "agreed"
        finally:
            verify_mod.CURATION_DIR = orig_dir
